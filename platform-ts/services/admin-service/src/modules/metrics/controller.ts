...
export const registerMetricsController = (app: FastifyInstance) => {
  app.get('/api/admin/metrics/http', { preHandler: app.requireRole('hr_admin') }, async () => {
    return httpMetrics(app);
  });

  app.get<{ Querystring: MetricsQuery }>('/api/admin/metrics/queues', { preHandler: app.requireRole('hr_admin') }, async (req) => {
