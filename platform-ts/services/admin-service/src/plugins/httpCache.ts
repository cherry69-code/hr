...
const ttlFor = (url: string) => {
  ...
  if (!url.startsWith('/api/admin')) return 0;
  if (url.startsWith('/api/admin/metrics')) return 5;
  if (url.startsWith('/api/admin/leaderboard')) return 10;
  return 10;
};
...
export const httpCachePlugin = fp(async (app) => {
  app.addHook('onRequest', async (req: any, reply) => {
    ...
    const key = `http:admin:${tenantId}:${role}:${url}`;
  ...
  app.addHook('onSend', async (req: any, reply, payload) => {
    ...
    if (typeof payload === 'string' && payload.length <= 500000) {
      ...
      await cacheTagAdd(app.redis, key, [`http:admin:${tenantId}`, `http:admin:${tenantId}:${role}`], ttl);
    }
    return payload;
  });
});
