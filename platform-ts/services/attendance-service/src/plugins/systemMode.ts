...
export const systemModePlugin = fp(async (app) => {
  ...
  app.redisSub.on('message', (_channel, message) => {
    if (_channel !== 'system.mode.changed') return;
    let parsed: any = null;
    try {
  ...
  app.addHook('onRequest', async (req, reply) => {
    ...
    if (url.startsWith('/admin/system/mode')) return;
    ...
    if (mode === 'maintenance') {
      reply.code(503).send({ ok: false, error: 'maintenance' });
    }
  });
});
